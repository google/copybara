...
<span id=core.workflow.set_rev_id href=#core.workflow.set_rev_id>set_rev_id</span> | <code><a href="#bool">bool</a></code><br><p>Copybara adds labels like 'GitOrigin-RevId' in the destination in order to track what was the latest change imported. This field allows disabling revision ID tracking. Note that disabling this may affect incremental migrations in `SQUASH` and `ITERATIVE` modes as Copybara won't be able to track what changes have already been migrated. For `CHANGE_REQUEST` workflows, revision IDs are purely informational. Destinations might ignore the flag.</p>
...
<span style="white-space: nowrap;">`--git-log-limit`</span> | *integer* | Limit log lines to the number specified
