  ...
  public Reader<RemoteArchiveRevision> newReader(Glob originFiles, Authoring authoring) {
    return new Reader<RemoteArchiveRevision>() {
      ...
      public ChangesResponse<RemoteArchiveRevision> changes(
          @Nullable RemoteArchiveRevision fromRef, RemoteArchiveRevision toRef)
        ...
        if (Strings.isNullOrEmpty(fromRef.fixedReference())
            || Strings.isNullOrEmpty(toRef.fixedReference())) {
          generalOptions
              .console()
              .warnFmt(
                  "Either the baseline ref[%s] or the incoming ref[%s] form as a fixed ref were not"
