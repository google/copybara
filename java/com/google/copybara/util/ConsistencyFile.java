  ...
  public void validateDirectory(ImmutableSet<String> files, HashGetter hashFetcher)
    ...
    if (!directoryOnlyFiles.isEmpty()) {
      throw new ValidationException(
          String.format(
              "Encountered files in directory not present in ConsistencyFile: %s",
              directoryOnlyFiles));
    }
    ...
    if (!consistencyFileOnlyHashes.isEmpty()) {
      throw new ValidationException(
          String.format(
              "Encountered files not found in directory but present in ConsistencyFile: %s",
              consistencyFileHashes));
    }
    ...
    for (Entry<String, String> hashEntry : fileHashes.entrySet()) {
      ...
      if (!expectedHash.equals(actualHash)) {
        throw new ValidationException(
            String.format(
                "File %s has hash value %s in ConsistencyFile but %s in directory",
