  ...
  @StarlarkMethod(
      ...
      parameters = {
        ...
        @Param(
            ...
            positional = false),
        @Param(
            name = "patch_file_path",
            doc =
                """
                A path to the patch file in which consistency diffs get written into when not \
                using merge import mode. When using merge import mode, diffs are written into the \
                consistency file and this parameter is ignored. If None, defaults to the \
                consistency file name with an additional '.patch' suffix.\
                """,
            allowedTypes = {
              @ParamType(type = String.class),
              @ParamType(type = NoneType.class),
            ...
            positional = false),
        @Param(
            name = "patch_path_prefix_to_strip",
            doc =
                """
                Path prefix to strip from file path headers in patch file generated by regenerate \
                command. \
                Use this together with the patch transformations' `directory` parameter to create \
                and apply patches relative to a directory instead of relative to destination root. \
                Always strips synthetic Copybara folders premerge/ and checkout/, even if None. \
                Has no effect on file headers in consistency file if merge import mode is used. \
                Example: value of "upstream_src/" would change file headers from \
                a/premerge/upstream_src/file to a/file and b/checkout/upstream_src/file to b/file.\
                """,
            allowedTypes = {
              @ParamType(type = String.class),
              @ParamType(type = NoneType.class),
