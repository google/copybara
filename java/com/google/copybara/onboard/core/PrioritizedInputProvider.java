...
/**
 * Given an {@link Input} and a {@code Collection} of {@link InputProvider}s for that Input, it
 * creates a InputProvider that calls the delegate InputProviders in priority order.
 */
public class PrioritizedInputProvider implements InputProvider {
  ...
  public PrioritizedInputProvider(Input<?> input, Collection<InputProvider> providers)
    ...
    for (InputProvider provider : providers) {
      this.providers.add(
          new PrioritizedEntry(
              provider,
              checkNotNull(
                  provider.provides().get(input),
                  "Provider %s doesn't provide %s",
                  provider,
                  input)));
    }
  }
  ...
  private static final class PrioritizedEntry implements Comparable<PrioritizedEntry> {
